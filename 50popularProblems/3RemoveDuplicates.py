...
def main4(arr: List[int]) -> List[int]:
    visited = {}
    for element in arr:
...
def main5(arr: List[int]) -> List[int]:
    return [val for idx, val in enumerate(arr) if val not in arr[:idx]]


#  if __name__ == '__main__': の分の中の処理は python3 sorts.py とコマンドで呼び出した時に自動的に呼ばれる
if __name__ == '__main__':
    s = main5([1, 2, 3, 4, 3, 2, 2, 5, 6, 7, 3, 4])
    print(s)
