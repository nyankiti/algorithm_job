...
public:
    ...
    void advance(const int action, bool is_adopted = false) {
...
int beamSearchAction(const GameState state, const int beam_width, const int beam_depth) {
    priority_queue<GameState> now_beam;
    GameState best_state;

    now_beam.push(state);
    for (int t = 0; t < beam_depth; t++) {
        priority_queue<GameState> next_beam;
        for (int i = 0; i < beam_width; i++) {
            if (now_beam.empty()) {
                break;
            }
            GameState now_state = now_beam.top();
            auto legal_actions = now_state.legalActions();
            if (legal_actions.size() > 0) {
                now_beam.pop();
                for (const auto &action : legal_actions) {
                    GameState next_state = now_state;
        ...
        if (point == 0)
            ...
            while (true) {
                ...
                if (legal_actions.size() == 0) {
                    ...
                    // ビームサーチ
                    gameState.advance(beamSearchAction(gameState, /*beam幅*/ 2, /*ビームの深さ*/ 2), true);
                }
            }
        }
    }
