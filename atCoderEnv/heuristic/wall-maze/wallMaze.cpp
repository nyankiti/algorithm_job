        ...
        while (!que.empty()) {
            ...
            for (int action = 0; action < 4; action++) {
                ...
                if (ty >= 0 && ty < H && tx >= 0 && tx < W && !this->walls_[ty][tx] && !check[ty][tx]) {
                    ...
                    // 以下の処理を高速化したのが上の処理
                    // que.emplace_back(DistanceCoord{ty, tx, tmp_cod.distance_ + 1});
                }
            }
        }
        return H * W;
...
int main() {
    ...
    const auto &ai = StringAIPair("beamSearchAction", [&](const State &state) { return beamSearchAction(state, beamwidth, beamdepth); });
